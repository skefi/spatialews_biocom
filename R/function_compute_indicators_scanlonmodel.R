#' Function which computes all the spatial metrics in the outcomes of simulations from the mode of Scanlon et al. 2007, Nature
#'
#' @param NPERM
#' @param N_SHUFFLE
#' @param path_output
...
#' @export
#'
#' @examples function_compute_indicators_scanlonmodel(NPERM,N_SHUFFLE,path_output,path_model_data)


function_compute_indicators_scanlonmodel <- function(NPERM,N_SHUFFLE,path_output,path_model_data){
  ...
  # Read the data
  scanlon_runs <- readRDS(file.path(path_model_data,"scanlon_ca_runs.rds"))
  scanlon_runs[[1]]$indics
  scanlon_runs[[1]]$mat

  # Extrait les matrices
  matrices <- lapply(scanlon_runs, function(o) o[["mat"]])
  model_params <- plyr::ldply(scanlon_runs, function(o) o[["covers"]])
