#' Function which estimates the trend (slope) of each of the spatial metrics along the aridity gradient in the 200 x 200 m images, in the whole data set or per group of sites
#'
#' @param NPERM 
#' @param N_SHUFFLE 
#' @param path_output 
...
#' @export 
#'
#' @examples function_indicator_trends_data200(NPERM,N_SHUFFLE,path_output,BOOTN,ALPHA)


function_indicator_trends_data200 <- function(NPERM,N_SHUFFLE,path_output,BOOTN,ALPHA){
  ...
  indics_fmt$aridity <- indics_fmt$Aridity # add aridity column because the get_slope function needs aridity without a capital A
  ...
  indics_fmt[ ,"pretty_grps2"] <- as.factor(indics_fmt[ ,"pretty_grps2"])
  ...
  indics_fmt[ ,"pretty_grps3"] <- as.factor(indics_fmt[ ,"pretty_grps3"])
