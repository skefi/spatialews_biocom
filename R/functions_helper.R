...
#' Function that computes the spatial metrics
#'
#' @param mat
#'
...
all_indicf <- function(mat,N_SHUFFLE) {
  ...
  #   cat(".")
  genindics <- c(spatialwarnings::raw_cg_moran(mat, subsize = 1),
  ...
  psdfit <- spatialwarnings::tpl_fit(psd)
...
# Comment this out as indic_groups is not always findable from the call to
# get_lm()
...
#' Function that resets the workers before a parallel computation. It is
#' useful to do so just before we spawn something in parallel, because the
#' cluster of workers may sometimes hang.
#' @param workers The number of workers to use
#' @return 
#'@export
reset_workers <- function(workers = parallel::detectCores() - 1) {
  library(future)
  library(future.apply)

  plan(sequential) # reset workers
  plan(multisession, workers = workers)
  future_lapply(seq.int(workers), function(i) {
    # This loads all functions into workers
    devtools::load_all()
  }, future.seed = TRUE)
