#' Function which computes all the spatial metrics in the 200 x 200 m images (subset of the Biocom data set)
#'
#' @param NPERM 
#' @param N_SHUFFLE
#' @param path_output 
...
#' @export 
#'
#' @examples function_compute_indicators_data200(NPERM,N_SHUFFLE,path_output)


function_compute_indicators_data200 <- function(NPERM,N_SHUFFLE,path_output){
  ...
  indic_results <- spatialwarnings::compute_indicator(ourdata_200x200[["matrices"]], 
                                                      fun = all_indicf,
                                                      N_SHUFFLE = N_SHUFFLE,
  ...
  reset_workers(workers=parallel::detectCores()-1)
  indic_results <- spatialwarnings::indictest(indic_results, nulln = NPERM, null_method = "perm")
  
  # Compute the flow length: the flowlength is a bit special because it is 
  ...
  has_info <- which( ! is.na(ourdata_200x200[["biocom"]][ , "resol"]) ) 
  
  fl_results <- plyr::ldply(has_info, function(i) { 
    ic_obs <- spatialwarnings::raw_flowlength_uniform(ourdata_200x200[["matrices"]][[i]], 
                                                      slope = ourdata_200x200[["biocom"]][i,"SLO"], 
                                                      cell_size = ourdata_200x200[["biocom"]][i,"resol"])
    ...
    data.frame(indic = "flowlength", 
               ourdata_200x200[["biocom"]][i,c("plotn", "plotid", "matrixn")], 
  ...
  indics_df[ ,"plotn"] <- ourdata_200x200[["biocom"]][indics_df[ ,"matrixn"], "plotn"]
  indics_df  <- dplyr::left_join( #add plotid image by image:
    indics_df,
    dplyr::select(ourdata_200x200[["biocom"]], # for all images
  ...
  indics_df <- plyr::join(indics_df, ourdata_200x200[["biocom"]], by = c("plotid", "plotn"))
