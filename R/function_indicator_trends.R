#' Function which the trend (slope) of each of the spatial metrics along the aridity gradient
#'
#' @param NPERM 
#' @param N_SHUFFLE 
#' @param path_output 
...
#' @export 
#'
#' @examples function_indicator_trends(NPERM,N_SHUFFLE,path_output,BOOTN,ALPHA)


function_indicator_trends <- function(NPERM,N_SHUFFLE,path_output,BOOTN,ALPHA){
  ...
  indic_groups <- c(variance = "Generic", 
                    moran    = "Generic", 
                    skewness = "Generic", 
                    sdr      = "Generic", 
                    cv.variance ="Generic",
                    maxpatch = "Patch-based",
                    fmaxpatch = "Patch-based",
                    logmaxpatch = "Patch-based",
                    logfmaxpatch = "Patch-based",
                    cutoff = "Patch-based",
                    slope    = "Patch-based", 
                    slope_pl    = "Patch-based", 
                    plrange  = "Patch-based",
                    pl_or_tpl   = "Patch-based", 
                    perco   = "Patch-based", 
                    flowlength  = "Hyd.",
                    ks_dist  = "Patch-based")
  ...
  indics_fmt <- subset(indics_fmt, indic != "maxpatch")
  indics_fmt <- subset(indics_fmt, indic != "logmaxpatch")
  indics_fmt <- subset(indics_fmt, indic != "fmaxpatch")
  indics_fmt <- subset(indics_fmt, indic != "variance")
  indics_fmt <- subset(indics_fmt, indic != "pl_or_tpl")
  indics_fmt <- subset(indics_fmt, indic != "perco")
  indics_fmt <- subset(indics_fmt, indic != "skewness")
  indics_fmt <- subset(indics_fmt, indic != "ks_dist")
  indics_fmt <- subset(indics_fmt, indic != "slope_pl")
